...
namespace CQRS.Business.Handlers.Concrete {
    public class UpdateDescTodoHandler : IRequestHandler<UpdateDescTodoRequest, TodosResponse> {

        private readonly ControletodosContext _repository;
        private readonly IDbContextTransaction _transaction;
        public UpdateDescTodoHandler(ControletodosContext context) {
            _repository = context;
            _transaction = context.Database.BeginTransaction();
        }
        public Task<TodosResponse> Handle(UpdateDescTodoRequest request, CancellationToken cancellationToken) {

            if (request.Id != 0 && !string.IsNullOrEmpty(request.Description)) {
